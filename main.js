...
var PDFAnnotationPluginSetting = class {
  constructor() {
    ...
    this.ext_fl_tog = false, this.ext_fl_suf = "(ext mm)", this.ext_es_tog = false, this.ext_es_suf = "(ext mm essential)";
    this.exportPath = "";
    ...
    this.lvl1_icon = "\u{1F7E0}";
    this.lvl2_icon = "\u{1F7E1}";
    this.lvl3_icon = "\u{1F535}";
    this.sumr_icon = "\u{1F7E2}";
    this.impt_icon = "\u{1F534}";
    this.ext_lvl1_icon = "\u{1F4CC}";
    this.ext_lvl2_icon = "";
    this.ext_lvl3_icon = "\u{1F537}";
    this.ext_sumr_icon = "\u{1F4DD}";
...
`;
    ...
    this.lnk_tog = false;
    this.lnk_cmd = " (p. {{page_number}}) [\u{1F517}](obsidian://shell-commands?execute=CMD_ID_FROM_SHELL_COMMANDS&_page_number={{page_number}}&_file_path={{file_path}})";
    this.parsedSettings = {
...
var PDFAnnotationPluginSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("1st page").setDesc("First page to extract annotations from. \nDefault: 0 (i.e. first page)").addText(
      (text) => text.setValue(this.plugin.settings.page_min.toString()).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.page_min = parseInt(value);
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Last page").setDesc("Last page to extract annotations up to. \nDefault: 0 (i.e. last page)").addText(
      (text) => text.setValue(this.plugin.settings.page_max.toString()).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.page_max = parseInt(value);
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Notes export path").setDesc("The path to which the notes, including the extracted annotations, will be exported. The path should be relative to the vault root. Paths must end with a '/'. Leave blank to export to the vault root.").addText((input) => this.buildValueInput(input, "exportPath"));
    containerEl.createEl("h3", { text: "Mindmaps to generate" });
    new import_obsidian.Setting(containerEl).setName("From PDF: Generate full Obsidian mindmap").setDesc("If enabled, generate the full mindmap for Obsidian when calling the plugin from a PDF file").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.mm_fl_tog).onChange((value) => {
        this.plugin.settings.mm_fl_tog = value;
        this.plugin.saveData(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("File suffixes: Full Obsidian mindmap").setDesc("File's suffixe for the Obsidian mindmap file.\nDefault: (mm)").addText(
      (text) => text.setValue(this.plugin.settings.mm_fl_suf).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.mm_fl_suf = value;
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Obsidian mindmap: Add a level").setDesc("If enabled, add a level containing all nodes to save space").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.mm_preamb).onChange((value) => {
        this.plugin.settings.mm_preamb = value;
        this.plugin.saveData(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("From PDF: Generate essentials Obsidian mindmap").setDesc("If enabled, generate the Obsidian mindmap with essentials (special levels) annotations when calling the plugin from a PDF file").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.mm_es_tog).onChange((value) => {
        this.plugin.settings.mm_es_tog = value;
        this.plugin.saveData(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("File suffixes: Obsidian Mindmap of essentials").setDesc("File's suffixe for the Obsidian mindmap with only essentials (special levels) annotations.\nDefault: (mm essential)").addText(
      (text) => text.setValue(this.plugin.settings.mm_es_suf).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.mm_es_suf = value;
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("From PDF: Generate full file for external mindmap").setDesc("If enabled, generate the full file for external mindmap when calling the plugin from a PDF file").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.ext_fl_tog).onChange((value) => {
        this.plugin.settings.ext_fl_tog = value;
        this.plugin.saveData(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("File suffixes: Full external-mindmap").setDesc("File's suffixe for the external-mindmap file.\nDefault: (ext mm)").addText(
      (text) => text.setValue(this.plugin.settings.ext_fl_suf).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.ext_fl_suf = value;
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("From PDF: Generate essentials file for external mindmap").setDesc("If enabled, generate the file for external mindmap with essentials (special levels) annotations when calling the plugin from a PDF file").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.ext_es_tog).onChange((value) => {
        this.plugin.settings.ext_es_tog = value;
        this.plugin.saveData(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("File suffixes: File for external mindmap of essentials").setDesc("File's suffixe for the external-mindmap with only essentials (special levels) annotations.\nDefault: (ext mm essential)").addText(
      (text) => text.setValue(this.plugin.settings.ext_es_suf).onChange((value) => __async(this, null, function* () {
    ...
    new import_obsidian.Setting(containerEl).setName("Color: Level 1").setDesc("R,G,B values for level 1 (separated by commas). \nDefault: Orange (255,173, 91)").addText(
      (text) => text.setValue(this.plugin.settings.level1RGB[0] + "," + this.plugin.settings.level1RGB[1] + "," + this.plugin.settings.level1RGB[2]).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.level1RGB = value.split(",").map(Number);
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Color: Level 2").setDesc("R,G,B values for level 2 (separated by commas).\nDefault: Yellow (255,255,  0)").addText(
      (text) => text.setValue(this.plugin.settings.level2RGB[0] + "," + this.plugin.settings.level2RGB[1] + "," + this.plugin.settings.level2RGB[2]).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.level2RGB = value.split(",").map(Number);
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Color: Level 3").setDesc("R,G,B values for level 3 (separated by commas).\nDefault: Light blue (209,223,235)").addText(
      (text) => text.setValue(this.plugin.settings.level3RGB[0] + "," + this.plugin.settings.level3RGB[1] + "," + this.plugin.settings.level3RGB[2]).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.level3RGB = value.split(",").map(Number);
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Color: Special level (1)").setDesc("R,G,B values for special level (1) such as summaries (separated by commas).\nDefault: Green (  0, 255,  0)").addText(
      (text) => text.setValue(this.plugin.settings.summryRGB[0] + "," + this.plugin.settings.summryRGB[1] + "," + this.plugin.settings.summryRGB[2]).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.summryRGB = value.split(",").map(Number);
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Color: Special level (2)").setDesc("R,G,B values for special level (2) such as important annot. (separated by commas).\nDefault: Red (252, 54, 54)").addText(
      (text) => text.setValue(this.plugin.settings.imprttRGB[0] + "," + this.plugin.settings.imprttRGB[1] + "," + this.plugin.settings.imprttRGB[2]).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.imprttRGB = value.split(",").map(Number);
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Color: Hue tolerance (%)").setDesc("Indicate the hue tolerance (in %) to recognize the colors.\nDefault: 5").addText(
      (text) => text.setValue(this.plugin.settings.hueTol.toString()).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.hueTol = parseInt(value);
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Color: Luminosity tolerance (%)").setDesc("Indicate the luminosity tolerance (in %) to recognize the colors.\nDefault: 30").addText(
      (text) => text.setValue(this.plugin.settings.LumiTol.toString()).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.LumiTol = parseInt(value);
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    containerEl.createEl("h4", { text: "LEVELS: Formatting" });
    new import_obsidian.Setting(containerEl).setName("Format: Level 1").setDesc("Indicate the format for level 1 annotations.\nDefault: [Empty]").addText(
      (text) => text.setValue(this.plugin.settings.lvl1_format).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.lvl1_format = value;
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Format: Level 2").setDesc("Indicate the format for level 2 annotations.\nDefault: [Empty]").addText(
      (text) => text.setValue(this.plugin.settings.lvl2_format).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.lvl2_format = value;
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Format: Level 3").setDesc("Indicate the format for level 3 annotations.\nDefault: [Empty]").addText(
      (text) => text.setValue(this.plugin.settings.lvl3_format).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.lvl3_format = value;
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Format: Summary").setDesc("Indicate the format for summary annotations.\nDefault: **").addText(
      (text) => text.setValue(this.plugin.settings.sumr_format).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.sumr_format = value;
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Format: Important").setDesc("Indicate the format for important annotations.\nDefault: ==").addText(
      (text) => text.setValue(this.plugin.settings.impt_format).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.impt_format = value;
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Format: Notes").setDesc("Indicate the format for notes.\nDefault: _").addText(
      (text) => text.setValue(this.plugin.settings.note_format).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.note_format = value;
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Preamble: Notes").setDesc("Indicate the notes'preamble.\nDefault: **Note:**").addText(
      (text) => text.setValue(this.plugin.settings.note_preamb).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.note_preamb = value + " ";
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    containerEl.createEl("h4", { text: "LEVELS: Icons" });
    new import_obsidian.Setting(containerEl).setName("Icons: Level 1").setDesc("Indicate the icon for level 1 annotations.\nDefault: \u{1F7E0}").addText(
      (text) => text.setValue(this.plugin.settings.lvl1_icon).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.lvl1_icon = value + " ";
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Icons: Level 2").setDesc("Indicate the icon for level 2 annotations.\nDefault: \u{1F7E1}").addText(
      (text) => text.setValue(this.plugin.settings.lvl2_icon).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.lvl2_icon = value + " ";
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Icons: Level 3").setDesc("Indicate the icon for level 3 annotations.\nDefault: \u{1F535}").addText(
      (text) => text.setValue(this.plugin.settings.lvl3_icon).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.lvl3_icon = value + " ";
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Icons: Summary").setDesc("Indicate the icon for summaries annotations.\nDefault: \u{1F7E2}").addText(
      (text) => text.setValue(this.plugin.settings.sumr_icon).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.sumr_icon = value + " ";
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Icons: Important").setDesc("Indicate the icon for important annotations.\nDefault: \u{1F534}").addText(
      (text) => text.setValue(this.plugin.settings.impt_icon).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.impt_icon = value + " ";
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Icons (ext. mm): Level 1").setDesc("Indicate the icon for level 1 annotations for external mindmap.\nDefault: \u{1F4CC}").addText(
      (text) => text.setValue(this.plugin.settings.ext_lvl1_icon).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.ext_lvl1_icon = value + " ";
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Icons (ext. mm): Level 2").setDesc("Indicate the icon for level 2 annotations for external mindmaps.\nDefault: [None]").addText(
      (text) => text.setValue(this.plugin.settings.ext_lvl2_icon).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.ext_lvl2_icon = value + " ";
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Icons (ext. mm): Level 3").setDesc("Indicate the icon for level 3 annotations for external mindmaps.\nDefault: \u{1F537}").addText(
      (text) => text.setValue(this.plugin.settings.ext_lvl3_icon).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.ext_lvl3_icon = value + " ";
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Icons (ext. mm): Summary").setDesc("Indicate the icon for summaries annotations for external mindmaps.\nDefault: \u{1F4DD}").addText(
      (text) => text.setValue(this.plugin.settings.ext_sumr_icon).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.ext_sumr_icon = value + " ";
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Icons (ext. mm): Important").setDesc("Indicate the icon for important annotations for external mindmaps.\nDefault: \u26A0\uFE0F").addText(
      (text) => text.setValue(this.plugin.settings.ext_impt_icon).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.ext_impt_icon = value + " ";
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Icons: Unknown").setDesc("Indicate the icon for unknown's level annotations.\nDefault: \u2754").addText(
      (text) => text.setValue(this.plugin.settings.unkn_icon).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.unkn_icon = value + " ";
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    containerEl.createEl("h4", { text: "FILES: Preambles" });
    new import_obsidian.Setting(containerEl).setName("Preambles: File beginning").setDesc(`Text inserted at the file's beginning. 
The marker {dateTime} will be replaced by
the date/Time in "DD/MM/YYYY @HH:mm" format`).addTextArea((text) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Preambles: PDF").setDesc("Text inserted at each new PDF.\nThe marker {fileName} will be replaced by\nthe PDF file name").addTextArea((text) => {
      text.inputEl.style.width = "100%";
    ...
    new import_obsidian.Setting(containerEl).setName("Preambles: Manual personal annotations").setDesc("Preamble for manual annotations to add to the extracted ones.\nDefault: ### Synth\xE8se perso").addText(
      (text) => text.setValue(this.plugin.settings.perso_prb).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.perso_prb = value;
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Preambles: Condensed").setDesc("Preamble for the condensed annotations.\nDefault: ### Format condens\xE9").addText(
      (text) => text.setValue(this.plugin.settings.conds_prb).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.conds_prb = value;
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Preambles: Detailed").setDesc("Preamble for the detailed annotations.\nDefault: ### Format d\xE9taill\xE9").addText(
      (text) => text.setValue(this.plugin.settings.detal_prb).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.detal_prb = value;
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    new import_obsidian.Setting(containerEl).setName("Preambles: No annotation").setDesc("Preamble in case there is no annotation.\nDefault: - **Aucune annotation**").addText(
      (text) => text.setValue(this.plugin.settings.no_an_prb).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.no_an_prb = value;
        yield this.plugin.saveData(this.plugin.settings);
      }))
    );
    containerEl.createEl("h4", { text: "LINK: Command" });
    containerEl.createEl("span", { text: "You can add here a command so that a link is created at the beginning of each annotation to open the PDF file at the annotation's page.To do this:" });
    ...
    containerEl.createEl("li", { text: "Copy the shell command by clicking on \u{1F517}." });
    ...
    new import_obsidian.Setting(containerEl).setName("Link-command: Add the command link").setDesc("If enabled, a link will be added to annotations as indicated below").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.lnk_tog).onChange((value) => {
        this.plugin.settings.lnk_tog = value;
        this.plugin.saveData(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("Link-command: Command to add").setDesc('Command to add to the link so that (for instance) the PDF file is opened at the specified page. Use {{page_number}} where the page should be placed, and {{file_path}} where the PDF file name/path should be placed. Eg.: " (p. {{page_number}}) [\u{1F517}](obsidian://shell-commands?execute=CMD_ID_FROM_SHELL_COMMANDS&_page_number={{page_number}}&_file_path={{file_path}})"').addTextArea(
      (text) => text.setValue(this.plugin.settings.lnk_cmd).onChange((value) => __async(this, null, function* () {
...
var COEFF_CRCT_LOW = 0;
var COEFF_CRCT_HIGH = 1;
function searchQuad(minx, maxx, miny, maxy, items) {
  const mycontent = items.reduce(function(txt, x) {
    ...
    if (x.transform[4] > maxx)
      return txt;
    const start = x.transform[4] >= minx ? 0 : Math.floor(
...
function extractHighlight(annot, items) {
  ...
  const highlight = legacyQuadPoints.reduce((txt, quad) => {
    ...
    const res = searchQuad(minx - COEFF_CRCT_LOW, maxx + COEFF_CRCT_HIGH, miny - COEFF_CRCT_LOW, maxy + COEFF_CRCT_LOW, items);
...
function loadPDFFile(file, page_min, page_max, pdfjsLib, containingFolder, total, desiredAnnotations) {
  return __async(this, null, function* () {
...
// src/saveToFile.ts
function saveDataToFile(filePath, dataStr) {
  return __async(this, null, function* () {
    let l_return = false;
    try {
      const fileExists = yield this.app.vault.adapter.exists(filePath);
      if (fileExists) {
        yield this.appendDataToFile(filePath, dataStr);
      } else {
        yield this.app.vault.create(filePath, dataStr);
      }
      l_return = true;
    } catch (error) {
...
var sumr_prefix = "- ";
var impt_prefix = "- ";
function getColorName(i_RGB) {
  let l_HSL = import_color_convert.default.rgb.hsl(i_RGB);
  let l_hue = l_HSL[0];
  let l_return = "";
  if (l_hue <= 20) {
    l_return = "Red";
  } else if (l_hue > 20 && l_hue <= 55) {
    l_return = "Orange";
  } else if (l_hue > 55 && l_hue <= 70) {
    l_return = "Yellow";
  } else if (l_hue > 70 && l_hue <= 160) {
    l_return = "Green";
  } else if (l_hue > 160 && l_hue <= 195) {
    l_return = "Cyan";
  } else if (l_hue > 195 && l_hue <= 240) {
    l_return = "Blue";
  } else if (l_hue > 240 && l_hue <= 270) {
    l_return = "Indigo";
  } else if (l_hue > 270 && l_hue <= 300) {
    l_return = "Violet";
  } else if (l_hue > 300 && l_hue <= 330) {
    l_return = "Magenta";
  } else {
    l_return = "Red";
  }
...
var PDFAnnotationPlugin = class extends import_obsidian2.Plugin {
  ...
  buildPreamble(currentFileName, i_isForObsMindmap, i_isForExtMindmap) {
  ...
  buildAnnotations(i_isForObsMindmap, i_isForExtMindmap, currentFileName, text_cd, text_dt) {
    let text = "";
  ...
  format(grandtotal, i_isForObsMindmap, i_isForExtMindmap, i_isGetNrml, i_isGetLow, isExternalFile) {
    let text = "";
    let topic = "";
    ...
    const color_lvl1 = import_color_convert.default.rgb.hsl(this.settings.level1RGB);
    const color_lvl2 = import_color_convert.default.rgb.hsl(this.settings.level2RGB);
    const color_lvl3 = import_color_convert.default.rgb.hsl(this.settings.level3RGB);
    const color_sumr = import_color_convert.default.rgb.hsl(this.settings.summryRGB);
    const color_impt = import_color_convert.default.rgb.hsl(this.settings.imprttRGB);
...
`;
      ...
      const l_date = /* @__PURE__ */ new Date();
...
`;
      ...
      l_annoToReport = true;
      let annotColor = [-1, -1, -1];
      if (anno.color != null) {
        annotColor = import_color_convert.default.rgb.hsl(anno.color);
      }
      let annotColorHue = annotColor[0];
      let annotColorLum = annotColor[2];
      l_note_sfx = "";
      if (Math.abs(100 * (annotColorHue - color_lvl1Hue) / color_maxHue) <= this.settings.hueTol && Math.abs(100 * (annotColorLum - color_lvl1Lum) / color_maxLum) <= this.settings.LumiTol) {
      ...
      } else if (Math.abs(100 * (annotColorHue - color_lvl2Hue) / color_maxHue) <= this.settings.hueTol && Math.abs(100 * (annotColorLum - color_lvl2Lum) / color_maxLum) <= this.settings.LumiTol) {
        if (i_isGetNrml) {
      ...
      } else if (Math.abs(100 * (annotColorHue - color_lvl3Hue) / color_maxHue) <= this.settings.hueTol && Math.abs(100 * (annotColorLum - color_lvl3Lum) / color_maxLum) <= this.settings.LumiTol) {
        if (i_isGetLow) {
      ...
      } else if (Math.abs(100 * (annotColorHue - color_sumrHue) / color_maxHue) <= this.settings.hueTol && Math.abs(100 * (annotColorLum - color_sumrLum) / color_maxLum) <= this.settings.LumiTol) {
      ...
      } else if (Math.abs(100 * (annotColorHue - color_imptHue) / color_maxHue) <= this.settings.hueTol && Math.abs(100 * (annotColorLum - color_imptLum) / color_maxLum) <= this.settings.LumiTol) {
      ...
      if (l_annoToReport) {
        ...
        if (i_isForExtMindmap == false) {
          ...
          } else if (l_details.includes("\r") || l_details.includes("\n")) {
  ...
  loadSinglePDFFile(file) {
    return __async(this, null, function* () {
      ...
      const pages = yield loadPDFFile(PDFFile.convertTFileToPDFFile(file, content), this.settings.page_min, this.settings.page_max, pdfjsLib, containingFolder, grandtotal, desiredAnnotations);
      ...
      yield saveDataToFile(l_fileName_1, finalMarkdown);
      yield this.app.workspace.openLinkText(l_fileName_1, "", true);
      if (this.settings.mm_fl_tog) {
        ...
        yield saveDataToFile(l_fileName_2, finalMarkdown);
        yield this.app.workspace.openLinkText(l_fileName_2, "", true);
      }
      if (this.settings.mm_es_tog) {
        ...
        yield saveDataToFile(l_fileName_3, finalMarkdown);
        yield this.app.workspace.openLinkText(l_fileName_3, "", true);
      }
      if (this.settings.ext_fl_tog) {
        ...
        yield saveDataToFile(l_fileName_4, finalMarkdown);
        yield this.app.workspace.openLinkText(l_fileName_4, "", true);
      }
      if (this.settings.ext_es_tog) {
        ...
        yield saveDataToFile(l_fileName_5, finalMarkdown);
        yield this.app.workspace.openLinkText(l_fileName_5, "", true);
      }
    });
  }
  loadSinglePDFFileClipboard(clipText) {
    return __async(this, null, function* () {
      const { pages, grandtotal } = yield this.loadAnnotationsFromSinglePDFFileFromClipboardPath(clipText);
      this.sort(grandtotal);
    ...
    return __async(this, null, function* () {
      ...
      try {
        ...
        if (stats.isFile()) {
          ...
          const desiredAnnotations = this.settings.parsedSettings.desiredAnnotations;
          pages = yield loadPDFFile(file, this.settings.page_min, this.settings.page_max, pdfjsLib, containingFolder, grandtotal, desiredAnnotations);
        } else {
  ...
  onload() {
    return __async(this, null, function* () {
      ...
      this.addCommand({
        ...
        name: "Extract PDF Annotations: On single file from path in clipboard \u{1F4CB}",
        editorCallback: (editor, view) => __async(this, null, function* () {
          let clipText = "";
          try {
            clipText = yield navigator.clipboard.readText();
          } catch (error) {
          ...
          import_obsidian2.Vault.recurseChildren(folder, (file2) => __async(this, null, function* () {
            if (file2 instanceof import_obsidian2.TFile) {
              if (file2.extension === "pdf") {
                promises.push(
                  this.app.vault.readBinary(file2).then((content) => loadPDFFile(PDFFile.convertTFileToPDFFile(file2, content), this.settings.page_min, this.settings.page_max, pdfjsLib, file2.parent.name, grandtotal, desiredAnnotations))
                );
              }
            }
          }));
  ...
  getContentForNoteFromExternalPDF(annotation) {
    ...
    if (this.settings.lnk_tog) {
      ...
      l_cmd = this.settings.lnk_cmd.replace(/{{page_number}}/g, anno.pageNumber.toString()).replace(/{{file_path}}/g, anno.filepath) + " ";
    }
